...
def main():
    ...
    module_spacing = CoulombForceModule(
        rcut=rcut, ethresh=ethresh, spacing=custom_spacing
    )
    energy_spacing = module_spacing(
        positions, box, pairs, ds, buffer_scales, {"charge": charges}
    )
    ...
    module_combined = CoulombForceModule(
        rcut=rcut, ethresh=ethresh, kappa=0.25, spacing=1.2, slab_corr=True, slab_axis=2
    )
    energy_combined = module_combined(
        positions, box, pairs, ds, buffer_scales, {"charge": charges}
    )
