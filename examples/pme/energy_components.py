...
def main():
    ...
    print(f"  Sum of components:      {(real_energy + reciprocal_energy + self_energy + non_neutral_energy).item():.6f} eV")
    
    # Verify that sum matches total
    assert torch.abs(energy - (real_energy + reciprocal_energy + self_energy + non_neutral_energy)) < 1e-10
    ...
    print(f"    Sum of components:      {(real_energy_slab + reciprocal_energy_slab + self_energy_slab + non_neutral_energy_slab + slab_corr_energy_slab).item():.6f} eV")
    
    # Verify that sum matches total
    assert torch.abs(energy_slab - (real_energy_slab + reciprocal_energy_slab + self_energy_slab + non_neutral_energy_slab + slab_corr_energy_slab)) < 1e-10
    print("    ✓ Verification passed: sum of components equals total energy")
    ...
    energy_high_ethresh = module_high_ethresh(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    ...
    energy_low_ethresh = module_low_ethresh(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    ...
    energy_custom_kappa = module_custom_kappa(positions, box, pairs, ds, buffer_scales, {"charge": charges})
