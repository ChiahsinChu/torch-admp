...
def main():
    ...
    try:
        module = CoulombForceModule(rcut=6.0, ethresh=1e-5, slab_corr=True, slab_axis=-1)
        print("   ERROR: Should have raised an exception!")
    except (ValueError, AssertionError, IndexError) as e:
        print(f"   ✓ Correctly caught error: {type(e).__name__}: {e}")
    ...
    try:
        ...
        non_orthogonal_box = np.array([[10.0, 1.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 10.0]])
    ...
    try:
        ...
        energy = module(positions, box, pairs, ds, buffer_scales, {"charge": wrong_charges})
        print("    ERROR: Should have raised an exception!")
    ...
    try:
        ...
        positions = torch.randn(100, 3, requires_grad=True)
        zero_volume_box = torch.diag(torch.tensor([10.0, 10.0, 0.0]))  # Zero in z dimension
        charges = torch.randn(100)
        ...
        energy = module(positions, zero_volume_box, pairs, ds, buffer_scales, {"charge": charges})
        print("    ERROR: Should have raised an exception!")
    except (RuntimeError, ValueError, AssertionError) as e:
        print(f"    ✓ Correctly caught error: {type(e).__name__}: {e}")
    ...
    for i, params in enumerate(valid_params):
        try:
            ...
            print(f"    ✗ Unexpected error with valid params {i+1}: {type(e).__name__}: {e}")
