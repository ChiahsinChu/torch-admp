...
def main():
    ...
    # Time JIT execution
    start_time = time.time()
    jit_energy = jit_module(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    for _ in range(9):  # Already computed once above
        jit_energy = jit_module(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    ...
    loaded_energy = loaded_jit_module(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    ...
    module_slab = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=True, slab_axis=2)
    jit_module_slab = torch.jit.script(module_slab)
    
    energy_slab = module_slab(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    jit_energy_slab = jit_module_slab(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    
    assert torch.abs(energy_slab - jit_energy_slab) < 1e-10
    print(f"  Slab correction - Regular: {energy_slab.item():.6f} eV, JIT: {jit_energy_slab.item():.6f} eV")
    ...
    jit_energy_kappa = jit_module_kappa(positions, box, pairs, ds, buffer_scales, {"charge": charges})
    
    assert torch.abs(energy_kappa - jit_energy_kappa) < 1e-10
    print(f"  Custom kappa - Regular: {energy_kappa.item():.6f} eV, JIT: {jit_energy_kappa.item():.6f} eV")
    ...
    for size in system_sizes:
        ...
        # Time regular execution
        start_time = time.time()
        energy_test = module_test(positions_test, box, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        ...
        # Time JIT execution
        start_time = time.time()
        jit_energy_test = jit_module_test(positions_test, box, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        for _ in range(4):  # Already computed once above
            jit_energy_test = jit_module_test(positions_test, box, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        ...
        print(f"  {size:3d} atoms: Regular {regular_time_test:.4f}s, JIT {jit_time_test:.4f}s, Speedup {regular_time_test/jit_time_test:.2f}x")
