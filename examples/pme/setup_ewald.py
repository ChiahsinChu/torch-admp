...
from torch_admp.pme import setup_ewald_parameters, CoulombForceModule
...
def main():
    ...
    for spacing_test in spacings:
        kappa_test, kx_test, ky_test, kz_test = setup_ewald_parameters(
            rcut=rcut, box=box, threshold=ethresh, spacing=spacing_test, method="gromacs"
        )
    ...
    energy_omm = module_omm(positions, box_tensor, pairs, ds, buffer_scales, {"charge": charges})
    
    # Test with Gromacs parameters
    module_gmx = CoulombForceModule(rcut=rcut, ethresh=ethresh, kappa=kappa_gmx)
    energy_gmx = module_gmx(positions, box_tensor, pairs, ds, buffer_scales, {"charge": charges})
    
    # Test with automatic parameters
    module_auto = CoulombForceModule(rcut=rcut, ethresh=ethresh)
    energy_auto = module_auto(positions, box_tensor, pairs, ds, buffer_scales, {"charge": charges})
