...
def main():
    ...
    for axis in [0, 1, 2]:
        module_axis = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=True, slab_axis=axis)
        energy_axis = module_axis(positions, box_2d, pairs, ds, buffer_scales, {"charge": charges})
    ...
    for ratio in thickness_ratios:
        ...
        module_test = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=True, slab_axis=2)
        energy_test = module_test(positions_test, box_2d, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        
        print(f"  Thickness ratio {ratio:.1f} (z < {l_z*ratio:.1f} Å): {energy_test.item():.6f} eV")
    ...
    for size in system_sizes:
        ...
        energy_3d_test = module_3d_test(positions_test, box_2d, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        
        # 2D slab correction
        module_2d_test = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=True, slab_axis=2)
        energy_2d_test = module_2d_test(positions_test, box_2d, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
    ...
    for i, (lx, ly, lz) in enumerate(aspect_ratios):
        ...
        positions_test = np.random.rand(n_atoms, 3) * [l_xy*lx, l_xy*ly, min_dim/3]
        positions_test = torch.tensor(positions_test, requires_grad=True)
        charges_test = torch.tensor(charges, requires_grad=False)
        ...
        # 3D PBC
        module_3d_test = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=False)
        energy_3d_test = module_3d_test(positions_test, box_test, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
        
        # 2D slab correction (z-axis)
        module_2d_test = CoulombForceModule(rcut=rcut, ethresh=ethresh, slab_corr=True, slab_axis=2)
        energy_2d_test = module_2d_test(positions_test, box_test, pairs_test, ds_test, buffer_scales_test, {"charge": charges_test})
    ...
    print(f"  Max force magnitude: {torch.max(torch.norm(forces_3d, dim=1)).item():.6f} eV/Å")
    print(f"  Mean force magnitude: {torch.mean(torch.norm(forces_3d, dim=1)).item():.6f} eV/Å")
    
    print(f"2D slab correction forces:")
    print(f"  Max force magnitude: {torch.max(torch.norm(forces_2d, dim=1)).item():.6f} eV/Å")
    print(f"  Mean force magnitude: {torch.mean(torch.norm(forces_2d, dim=1)).item():.6f} eV/Å")
